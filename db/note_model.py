...
sql = "select * from note where id_user = 1 and id_note = %d order by
tanggal_diperbaharui desc" % (id_note)
try :
self.cursor.execute(sql)
row = self.cursor.fetchone()
results = {}
results.update({'id_note':row[0], 'judul':row[2], 'tag':row[3], 'isi':row[4],
'tanggal_dibuat':row[5], 'tanggal_diubah':row[6]})
return results
except:
...
sql = "insert into note (id_user, judul, tag, isi, tanggal_dibuat, tanggal_diperbaharui) values
(%d, '%s', '%s', '%s', '%s', '%s')" % (1, judul, tag, isi, temp_tanggal.strftime('%Y-%m-%d %H-%M-
%S'), temp_tanggal.strftime('%Y-%m-%d %H-%M-%S'))
try:
self.cursor.execute(sql)
db.commit()
...
for row in temp_results:
num = i
results.append({'num':num, 'id_note':row[0], 'judul':row[2], 'tag':row[3], 'isi':row[4],
'tanggal_dibuat':row[5], 'tanggal_diubah':row[6]})
return results
except:
